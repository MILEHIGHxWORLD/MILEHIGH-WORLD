...
namespace MilehighWorld.World.Terminal
{
    public class OtisTerminal : MonoBehaviour
    {
        ...
        private string _inputBuffer = ""; // Palette: Temporary buffer to retain unsent text during history navigation.
        ...
        private void Update()
        {
            ...
            // Palette: Escape key clears the input line and resets state if not empty.
        ...
        private void NavigateHistory(int direction)
        {
            ...
            if (nextIndex == _historyIndex) return; // index didn't change, prevent premature buffer loss

            // If we are moving away from the current line (which is index == _commandHistory.Count), save buffer

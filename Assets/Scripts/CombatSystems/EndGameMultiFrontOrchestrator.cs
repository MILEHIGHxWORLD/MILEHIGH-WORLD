...
namespace MilehighWorld.CombatSystems
{
    public class EndGameMultiFrontOrchestrator : MonoBehaviour
    {
        ...
        public async Task CoordinateFinalNexusLockAsync(EncounterDirector director, LatticeSynchronizer synchronizer)
        {
            ...
            // ⚡ Bolt: Cache component lookups and material property IDs outside the loop
            // 💡 What: Extracted GetComponent and Shader.PropertyToID calls outside the while loop.
            // 🎯 Why: Calling GetComponent and performing string hashing inside a frame-bound loop causes unnecessary per-frame CPU/GC overhead.
            // 📊 Impact: Eliminates native/managed boundary crossing overhead and string hashing costs per frame.

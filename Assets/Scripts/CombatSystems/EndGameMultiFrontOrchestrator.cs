...
namespace MilehighWorld.CombatSystems
{
    public class EndGameMultiFrontOrchestrator : MonoBehaviour
    {
        ...
        public async Task CoordinateFinalNexusLockAsync(EncounterDirector director, LatticeSynchronizer synchronizer)
        {
            ...
            // ⚡ Bolt: Cache component lookups, dictionary lookups, and property IDs outside the per-frame loop.
            // 💡 What: Hoisted GetAlly, GetComponent, and Shader.PropertyToID calls out of the async while loop.
            // 🎯 Why: Executing dictionary lookups, GetComponent calls, and string-based property updates inside a frame-bound loop causes significant per-frame CPU overhead and string hashing overhead due to native/managed boundary crossings.
            // 📊 Impact: Eliminates redundant native/managed boundary crossings, improving framerate consistency.

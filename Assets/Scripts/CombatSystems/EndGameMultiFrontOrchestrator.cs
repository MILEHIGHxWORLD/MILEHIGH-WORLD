...
namespace MilehighWorld.CombatSystems
{
    public class EndGameMultiFrontOrchestrator : MonoBehaviour
    {
        ...
        public async Task CoordinateFinalNexusLockAsync(EncounterDirector director, LatticeSynchronizer synchronizer)
        {
            ...
            // ⚡ Bolt:
            // 💡 What: Hoisted GetAlly, GetComponent, and Shader.PropertyToID calls outside the loop.
            // 🎯 Why: Calling GetComponent, dictionary lookups, and string-based SetFloat inside a frame-bound loop causes high per-frame CPU and string hashing overhead.
            // 📊 Impact: Eliminates redundant C#/C++ native boundary crossings and reduces CPU overhead per frame.

...
namespace MilehighWorld.CombatSystems
{
    public class EndGameMultiFrontOrchestrator : MonoBehaviour
    {
        ...
        public async Task CoordinateFinalNexusLockAsync(EncounterDirector director, LatticeSynchronizer synchronizer)
        {
            ...
            // ⚡ Bolt Optimization
            // 💡 What: Hoisted `GetComponent<Rigidbody>`, `GetAlly` dictionary lookups, and `Shader.PropertyToID` hashing outside the `while` loop.
            // 🎯 Why: Executing these inside an async frame-bound loop causes significant CPU overhead and string hashing overhead due to native/managed boundary crossings.

...
namespace MilehighWorld.CombatSystems
{
    public class EndGameMultiFrontOrchestrator : MonoBehaviour
    {
        ...
        public async Task CoordinateFinalNexusLockAsync(EncounterDirector director, LatticeSynchronizer synchronizer)
        {
            ...
            // ⚡ Bolt: Hoisted frame-bound lookups out of the async loop
            // What: Cached GetAlly, GetComponent, and Shader.PropertyToID lookups before the while loop.
            // Why: Executing dictionary lookups, GetComponent, and string-based Material.SetFloat inside a frame-bound loop with Task.Yield() causes significant per-frame CPU and string hashing overhead due to native/managed boundary crossings.
            // Impact: Eliminates redundant O(N) traversals, native/managed boundary crossings, and string hashing per frame.

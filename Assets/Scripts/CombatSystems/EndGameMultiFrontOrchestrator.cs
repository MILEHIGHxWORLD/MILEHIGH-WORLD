...
namespace MilehighWorld.CombatSystems
{
    public class EndGameMultiFrontOrchestrator : MonoBehaviour
    {
        ...
        public async Task CoordinateFinalNexusLockAsync(EncounterDirector director, LatticeSynchronizer synchronizer)
        {
            ...
            // ⚡ Bolt: Cache component lookups, dictionary lookups, and shader property IDs outside the frame-bound loop.
            // 💡 What: Hoisted GetComponent, GetAlly, and Shader.PropertyToID out of the async while loop.
            // 🎯 Why: Executing these lookups and string hashes per-frame with Task.Yield() causes significant CPU overhead and GC allocations.
            // 📊 Impact: Eliminates redundant native/managed boundary crossings and string hashing, reducing per-frame CPU time.
            var squadMassOverride = micahBulwark != null ? micahBulwark.PrefabReference.GetComponent<Rigidbody>() : null;

...
namespace MilehighWorld.CombatSystems
{
    public class EndGameMultiFrontOrchestrator : MonoBehaviour
    {
        ...
        public async Task CoordinateFinalNexusLockAsync(EncounterDirector director, LatticeSynchronizer synchronizer)
        {
            ...
            // ⚡ Bolt:
            // 💡 What: Hoisted dictionary lookup, GetComponent, and Shader.PropertyToID calls outside the loop.
            // 🎯 Why: Executing these inside an async while loop with Task.Yield() causes per-frame CPU and string hashing overhead due to native/managed boundary crossings.
            // 📊 Impact: Eliminates redundant dictionary traversals, native component lookups, and string hashing each frame, reducing CPU overhead and preserving frame rate.

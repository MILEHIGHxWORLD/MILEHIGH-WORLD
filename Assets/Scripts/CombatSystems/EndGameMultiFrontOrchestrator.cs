...
namespace MilehighWorld.CombatSystems
{
    public class EndGameMultiFrontOrchestrator : MonoBehaviour
    {
        ...
        public async Task CoordinateFinalNexusLockAsync(EncounterDirector director, LatticeSynchronizer synchronizer)
        {
            ...
            // ⚡ Bolt: Cache component lookups, dictionary access, and property IDs outside the loop
            // 💡 What: Hoisted GetComponent, GetAlly, and Shader.PropertyToID calls out of the per-frame loop.
            // 🎯 Why: Executing these inside an async while loop containing await Task.Yield() causes significant per-frame CPU overhead and string hashing overhead due to native/managed boundary crossings.
            // 📊 Impact: Eliminates redundant native/managed boundary crossings and string hashing per frame, reducing CPU overhead and micro-stutters.

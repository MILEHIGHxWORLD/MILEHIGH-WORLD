...
namespace MilehighWorld.CombatSystems
{
    public class EndGameMultiFrontOrchestrator : MonoBehaviour
    {
        ...
        public async Task CoordinateFinalNexusLockAsync(EncounterDirector director, LatticeSynchronizer synchronizer)
        {
            ...
            // ⚡ Bolt: Hoisted expensive lookups and string hashes outside the async loop.
            // 💡 What: Cached GetComponent, dictionary lookups (GetAlly), and Shader.PropertyToID outside the loop.
            // 🎯 Why: Executing these inside an async loop with Task.Yield() causes per-frame CPU overhead and string hashing overhead.
            // 📊 Impact: Eliminates redundant per-frame GC allocations and native/managed boundary crossings.

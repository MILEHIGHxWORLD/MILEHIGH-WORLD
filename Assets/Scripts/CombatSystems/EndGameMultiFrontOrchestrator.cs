...
namespace MilehighWorld.CombatSystems
{
    public class EndGameMultiFrontOrchestrator : MonoBehaviour
    {
        ...
        public async Task CoordinateFinalNexusLockAsync(EncounterDirector director, LatticeSynchronizer synchronizer)
        {
            ...
            // ⚡ Bolt:
            // 💡 What: Cache Rigidbody component, dictionary lookup (Reverie), and Shader Property IDs outside the loop.
            // 🎯 Why: Calling GetComponent, executing dictionary lookups, or performing string-based property updates inside a frame-bound loop (like while with Task.Yield) causes significant CPU and string hashing overhead per frame.
            // 📊 Impact: Eliminates redundant native/managed boundary crossings, allocations, and string hashes every frame.

...
namespace MilehighWorld.CombatSystems
{
    public class EndGameMultiFrontOrchestrator : MonoBehaviour
    {
        ...
        public async Task CoordinateFinalNexusLockAsync(EncounterDirector director, LatticeSynchronizer synchronizer)
        {
            ...
            // ⚡ Bolt:
            // 💡 What: Hoisted GetComponent, dictionary lookups, and string hashing out of the frame-bound while loop.
            // 🎯 Why: Calling GetComponent, GetAlly, and Material.SetFloat(string) inside a frame-bound loop causes significant CPU and GC overhead due to repeated native/managed crossings and string hashing.
            // 📊 Impact: Eliminates redundant O(N) searches and string hashing per frame, smoothing framerate during the convergence sequence.

...
namespace MilehighWorld.CombatSystems
{
    public class EndGameMultiFrontOrchestrator : MonoBehaviour
    {
        ...
        public async Task CoordinateFinalNexusLockAsync(EncounterDirector director, LatticeSynchronizer synchronizer)
        {
            ...
            // ⚡ Bolt:
            // 💡 What: Hoisted Rigidbody GetComponent, GetAlly lookup, and Shader.PropertyToID calls outside the async while loop.
            // 🎯 Why: Calling GetComponent, dictionary lookups, and string-based Material.SetFloat inside a frame-bound loop (await Task.Yield) causes significant per-frame CPU and string hashing overhead due to native/managed boundary crossings.
            // 📊 Impact: Eliminates redundant O(N) component searches, string hashing, and GC allocations per frame, smoothing framerate during the cinematic endgame sequence.

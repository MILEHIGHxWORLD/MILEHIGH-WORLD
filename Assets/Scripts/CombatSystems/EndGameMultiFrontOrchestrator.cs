...
namespace MilehighWorld.CombatSystems
{
    public class EndGameMultiFrontOrchestrator : MonoBehaviour
    {
        ...
        public async Task CoordinateFinalNexusLockAsync(EncounterDirector director, LatticeSynchronizer synchronizer)
        {
            ...
            // ⚡ Bolt: Cache component lookups, dictionary access, and Shader property IDs outside the loop
            // 💡 What: Hoisted GetComponent, GetAlly, and string-based SetFloat lookups out of the per-frame await Task.Yield loop.
            // 🎯 Why: Repeatedly calling GetComponent, dictionary lookups, and hashing string shader properties inside a frame-bound loop causes high CPU overhead and unnecessary managed/native crossings.
            // 📊 Impact: Eliminates redundant O(N) traversals, string hashing, and GC allocations per frame, stabilizing frame times.

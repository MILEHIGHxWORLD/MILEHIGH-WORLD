...
namespace MilehighWorld.CombatSystems
{
    public class EndGameMultiFrontOrchestrator : MonoBehaviour
    {
        ...
        public async Task CoordinateFinalNexusLockAsync(EncounterDirector director, LatticeSynchronizer synchronizer)
        {
            ...
            // ⚡ Bolt:
            // 💡 What: Hoisted GetComponent, dictionary lookups, and Shader.PropertyToID outside the loop.
            // 🎯 Why: Executing these inside an async while loop with Task.Yield() causes significant per-frame CPU and string hashing overhead.
            // 📊 Impact: Eliminates redundant native/managed boundary crossings and dictionary lookups per frame.

...
namespace MilehighWorld.CombatSystems
{
    public class EndGameMultiFrontOrchestrator : MonoBehaviour
    {
        ...
        public async Task CoordinateFinalNexusLockAsync(EncounterDirector director, LatticeSynchronizer synchronizer)
        {
            ...
            // ⚡ Bolt: Cache lookups outside the loop to eliminate per-frame overhead
            // 💡 What: Hoisted GetAlly, GetComponent, and Shader.PropertyToID calls out of the loop.
            // 🎯 Why: Dictionary lookups, GetComponent calls, and string hashing inside an await Task.Yield() loop cause significant CPU/GC overhead.
            // 📊 Impact: Eliminates redundant native boundary crossings, string hashing, and lookups every frame during the convergence loop.

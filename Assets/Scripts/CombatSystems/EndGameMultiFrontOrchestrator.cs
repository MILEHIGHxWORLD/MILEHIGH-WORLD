...
namespace MilehighWorld.CombatSystems
{
    public class EndGameMultiFrontOrchestrator : MonoBehaviour
    {
        ...
        public async Task CoordinateFinalNexusLockAsync(EncounterDirector director, LatticeSynchronizer synchronizer)
        {
            ...
            // ⚡ Bolt:
            // 💡 What: Hoisted dictionary lookups (GetAlly), GetComponent calls, and Shader.PropertyToID caches outside the per-frame loop.
            // 🎯 Why: Repeatedly executing these inside a frame-bound loop (while with Task.Yield) introduces severe native/managed boundary crossing overhead and string hashing costs.
            // 📊 Impact: Significantly reduces per-frame CPU time, eliminates redundant string hashing, and prevents potential micro-stutters.

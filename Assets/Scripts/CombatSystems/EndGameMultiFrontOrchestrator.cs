...
namespace MilehighWorld.CombatSystems
{
    public class EndGameMultiFrontOrchestrator : MonoBehaviour
    {
        ...
        public async Task CoordinateFinalNexusLockAsync(EncounterDirector director, LatticeSynchronizer synchronizer)
        {
            ...
            // ⚡ Bolt: Cache component and property IDs outside the loop
            // 💡 What: Hoisted GetComponent, GetAlly, and Shader.PropertyToID lookups outside the per-frame loop.
            // 🎯 Why: Calling GetComponent or executing string-based material property updates inside an async frame-bound loop introduces massive per-frame CPU overhead due to native/managed C++ boundary crossings.
            // 📊 Impact: Eliminates repeated C++ crossings, reducing CPU overhead and string hashing costs per frame.

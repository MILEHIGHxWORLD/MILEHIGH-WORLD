...
namespace MilehighWorld.CombatSystems
{
    public class EndGameMultiFrontOrchestrator : MonoBehaviour
    {
        ...
        public async Task CoordinateFinalNexusLockAsync(EncounterDirector director, LatticeSynchronizer synchronizer)
        {
            ...
            // ⚡ Bolt: Cache component lookups, dictionary lookups, and shader IDs outside loop
            // 💡 What: Hoisted Rigidbody GetComponent, GetAlly("Reverie"), and Shader.PropertyToID calls outside the while loop.
            // 🎯 Why: Executing lookups and string-hashing inside a frame-bound async loop causes significant per-frame CPU overhead due to native/managed boundary crossings.
            // 📊 Impact: Eliminates O(N) traversal overhead and string hashing per frame during the convergence loop.

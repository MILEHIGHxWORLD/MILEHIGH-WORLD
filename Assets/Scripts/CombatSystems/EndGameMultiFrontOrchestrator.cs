...
namespace MilehighWorld.CombatSystems
{
    public class EndGameMultiFrontOrchestrator : MonoBehaviour
    {
        ...
        public async Task CoordinateFinalNexusLockAsync(EncounterDirector director, LatticeSynchronizer synchronizer)
        {
            ...
            // ⚡ Bolt: Hoisted GetComponent, GetAlly, and PropertyToID lookups outside the frame-bound Task.Yield loop.
            // 💡 What: Cached the Rigidbody, Reverie ally reference, and Shader property IDs before the while loop.
            // 🎯 Why: Calling GetComponent, dictionary lookups, and string-based SetFloat inside a loop containing await Task.Yield() causes significant per-frame CPU overhead and string hashing overhead due to native/managed boundary crossings.
            // 📊 Impact: Eliminates redundant native traversals and string hashing per frame, reducing CPU spikes during the cinematic convergence sequence.

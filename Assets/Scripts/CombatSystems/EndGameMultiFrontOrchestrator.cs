...
namespace MilehighWorld.CombatSystems
{
    public class EndGameMultiFrontOrchestrator : MonoBehaviour
    {
        ...
        public async Task CoordinateFinalNexusLockAsync(EncounterDirector director, LatticeSynchronizer synchronizer)
        {
            ...
            // ⚡ Bolt Optimization:
            // 💡 What: Hoisted dictionary lookups (`GetAlly`), component getters (`GetComponent`), and string-based property ID lookups (`Shader.PropertyToID`) outside of the frame-bound async loop.
            // 🎯 Why: Repeatedly executing these operations inside an async loop with `Task.Yield()` introduces severe CPU overhead per frame due to redundant native/managed boundary crossings and string hashing.
            // 📊 Impact: Eliminates O(N) traversals, redundant boundary crossings, and string hashing overhead per frame.
            var reverie = director.GetAlly("Reverie");

            Rigidbody squadMassOverride = null;
            if (micahBulwark != null)
            {

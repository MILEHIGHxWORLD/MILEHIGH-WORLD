...
namespace MilehighWorld.CombatSystems
{
    public class EndGameMultiFrontOrchestrator : MonoBehaviour
    {
        ...
        public async Task CoordinateFinalNexusLockAsync(EncounterDirector director, LatticeSynchronizer synchronizer)
        {
            ...
            // ⚡ Bolt:
            // 💡 What: Hoisted GetComponent, GetAlly, and Shader.PropertyToID out of the frame-bound while loop.
            // 🎯 Why: Dictionary lookups, GetComponent calls, and string-based property updates inside a loop with Task.Yield() cause significant per-frame CPU overhead and GC allocations.
            // 📊 Impact: Eliminates redundant native/managed boundary crossings and string hashing overhead per frame.

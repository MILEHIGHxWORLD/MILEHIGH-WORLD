...
namespace MilehighWorld.CombatSystems
{
    public class EndGameMultiFrontOrchestrator : MonoBehaviour
    {
        ...
        public async Task CoordinateFinalNexusLockAsync(EncounterDirector director, LatticeSynchronizer synchronizer)
        {
            ...
            // ⚡ Bolt: Cache component lookups, dictionary lookups, and shader property IDs outside the frame-bound loop to eliminate per-frame overhead.
            // 💡 What: Hoisted GetAlly("Reverie"), GetComponent<Rigidbody>(), and Shader.PropertyToID calls outside the while loop.
            // 🎯 Why: Executing string hashing, dictionary lookups, and native/managed boundary crossings inside a Task.Yield loop causes significant CPU overhead and GC allocations.
            // 📊 Impact: Eliminates redundant O(N) operations and native boundary crossings per frame.

...
namespace MilehighWorld.CombatSystems
{
    public class EndGameMultiFrontOrchestrator : MonoBehaviour
    {
        ...
        public async Task CoordinateFinalNexusLockAsync(EncounterDirector director, LatticeSynchronizer synchronizer)
        {
            ...
            // ⚡ Bolt: Cache component lookups, dictionary lookups, and shader property IDs outside the loop
            // 💡 What: Hoisted Rigidbody GetComponent, GetAlly lookup, and Shader.PropertyToID calls outside the while loop.
            // 🎯 Why: Executing these lookups and string hashing per-frame inside an async loop causes significant CPU overhead and native/managed boundary crossings.
            // 📊 Impact: Eliminates redundant native calls, string hashing, and dictionary lookups per frame.

...
namespace MilehighWorld.CombatSystems
{
    public class EndGameMultiFrontOrchestrator : MonoBehaviour
    {
        ...
        public async Task CoordinateFinalNexusLockAsync(EncounterDirector director, LatticeSynchronizer synchronizer)
        {
            ...
            // ⚡ Bolt: Cache component lookups and string-to-IDs outside frame-bound loop
            // 💡 What: Hoisted GetAlly, GetComponent, and Shader.PropertyToID out of the Task.Yield() loop.
            // 🎯 Why: Repeatedly crossing the native/managed boundary and allocating strings inside an async while loop causes immense CPU overhead per frame.
            // 📊 Impact: Significantly reduces per-frame execution time and prevents string-hashing overhead.

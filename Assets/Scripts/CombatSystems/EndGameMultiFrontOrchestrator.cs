...
namespace MilehighWorld.CombatSystems
{
    public class EndGameMultiFrontOrchestrator : MonoBehaviour
    {
        ...
        public async Task CoordinateFinalNexusLockAsync(EncounterDirector director, LatticeSynchronizer synchronizer)
        {
            ...
            // ⚡ Bolt:
            // 💡 What: Hoist and cache dictionary lookups, GetComponents, and Shader property IDs.
            // 🎯 Why: Prevents severe per-frame CPU overhead and string hashing at the native/managed boundary during the await Task.Yield() loop.
            // 📊 Impact: Eliminates O(N) lookup and string hashing overhead per frame.
            var reverie = director.GetAlly("Reverie");
            Rigidbody squadMassOverride = micahBulwark != null && micahBulwark.PrefabReference != null

...
namespace MilehighWorld.CombatSystems
{
    public class EndGameMultiFrontOrchestrator : MonoBehaviour
    {
        ...
        public async Task CoordinateFinalNexusLockAsync(EncounterDirector director, LatticeSynchronizer synchronizer)
        {
            ...
            // ⚡ Bolt: Cache component lookups, dictionary lookups, and property IDs outside the loop
            // 💡 What: Hoisted Rigidbody GetComponent, GetAlly dictionary lookup, and Shader.PropertyToID calls.
            // 🎯 Why: Executing these lookups and string-based property updates inside a frame-bound loop causes high CPU overhead.
            // 📊 Impact: Eliminates redundant native/managed boundary crossings and string hashing per frame.

...
namespace MilehighWorld.CombatSystems
{
    public class EndGameMultiFrontOrchestrator : MonoBehaviour
    {
        ...
        public async Task CoordinateFinalNexusLockAsync(EncounterDirector director, LatticeSynchronizer synchronizer)
        {
            ...
            // ⚡ Bolt Optimization
            // 💡 What: Hoisted GetAlly, GetComponent, and Shader.PropertyToID lookups outside the frame-bound loop.
            // 🎯 Why: Dictionary lookups, GetComponent calls, and string-based property updates inside an async while loop (with Task.Yield) cause per-frame CPU and string hashing overhead due to native/managed boundary crossings.
            // 📊 Impact: Eliminates redundant native boundary crossing and string hashing overhead per frame.

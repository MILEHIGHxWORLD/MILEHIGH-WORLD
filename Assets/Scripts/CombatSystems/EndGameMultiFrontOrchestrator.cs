...
namespace MilehighWorld.CombatSystems
{
    public class EndGameMultiFrontOrchestrator : MonoBehaviour
    {
        ...
        public async Task CoordinateFinalNexusLockAsync(EncounterDirector director, LatticeSynchronizer synchronizer)
        {
            ...
            // ⚡ Bolt: Hoisted GetComponent, string hashing, and lookups outside the loop.
            // 💡 What: Cached Rigidbody, Reverie, and Shader.PropertyToID before the loop.
            // 🎯 Why: Repeated lookups and string-based property updates in a frame-bound loop cause significant CPU and GC overhead.
            // 📊 Impact: Eliminates native boundary crossing and string hashing overhead per iteration.
            Rigidbody squadMassOverride = micahBulwark?.PrefabReference?.GetComponent<Rigidbody>();

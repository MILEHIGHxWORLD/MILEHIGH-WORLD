...
namespace MilehighWorld.CombatSystems
{
    public class EndGameMultiFrontOrchestrator : MonoBehaviour
    {
        ...
        public async Task CoordinateFinalNexusLockAsync(EncounterDirector director, LatticeSynchronizer synchronizer)
        {
            ...
            // 💡 What: Hoisted GetComponent<Rigidbody>(), dictionary lookup (GetAlly), and Shader.PropertyToID calls outside the async loop.
            // 🎯 Why: Prevents per-frame native/managed boundary crossings, dictionary hashing overhead, and redundant string hashing.
            // 📊 Impact: Eliminates recurring GC allocations and reduces per-frame CPU time significantly.

...
namespace MilehighWorld.CombatSystems
{
    public class EndGameMultiFrontOrchestrator : MonoBehaviour
    {
        ...
        public async Task CoordinateFinalNexusLockAsync(EncounterDirector director, LatticeSynchronizer synchronizer)
        {
            ...
            // ⚡ Bolt:
            // 💡 What: Hoisted dictionary lookups, GetComponent calls, and Shader string hashing outside the per-frame loop.
            // 🎯 Why: Executing GetComponent, dictionary lookups, and string-based Material.SetFloat inside an async while loop containing await Task.Yield() causes significant per-frame CPU and string hashing overhead due to native/managed boundary crossings.
            // 📊 Impact: Eliminates redundant O(N) component traversals, dictionary lookups, and string hashing per frame.

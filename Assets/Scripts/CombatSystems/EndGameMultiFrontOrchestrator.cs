...
namespace MilehighWorld.CombatSystems
{
    public class EndGameMultiFrontOrchestrator : MonoBehaviour
    {
        ...
        public async Task CoordinateFinalNexusLockAsync(EncounterDirector director, LatticeSynchronizer synchronizer)
        {
            ...
            // ⚡ Bolt: Cache component lookups and property IDs outside the frame-bound loop
            // 💡 What: Hoisted GetComponent, dictionary lookups, and Shader.PropertyToID out of the async while loop.
            // 🎯 Why: Repeated lookups and string hashing inside a per-frame loop cause high CPU overhead due to native/managed boundary crossings.
            // 📊 Impact: Eliminates O(N) GC allocations and string hashing per frame.

...
namespace MilehighWorld.CombatSystems
{
    public class EndGameMultiFrontOrchestrator : MonoBehaviour
    {
        ...
        public async Task CoordinateFinalNexusLockAsync(EncounterDirector director, LatticeSynchronizer synchronizer)
        {
            ...
            // ⚡ Bolt:
            // 💡 What: Hoisted GetComponent, GetAlly, and Shader.PropertyToID lookups outside the frame-bound loop.
            // 🎯 Why: Calling GetComponent, dictionary lookups, or string-based property updates inside an async frame-bound loop causes significant CPU overhead via native/managed crossings.
            // 📊 Impact: Eliminates redundant per-frame GC allocations and string hashing, reducing frame time overhead.

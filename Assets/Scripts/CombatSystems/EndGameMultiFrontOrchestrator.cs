...
namespace MilehighWorld.CombatSystems
{
    public class EndGameMultiFrontOrchestrator : MonoBehaviour
    {
        ...
        public async Task CoordinateFinalNexusLockAsync(EncounterDirector director, LatticeSynchronizer synchronizer)
        {
            ...
            // ⚡ Bolt:
            // 💡 What: Hoisted GetComponent, dictionary lookup (GetAlly), and property ID hashing outside the loop.
            // 🎯 Why: Calling these inside a frame-bound async loop causes significant per-frame CPU and string hashing overhead.
            // 📊 Impact: Eliminates per-frame native/managed boundary crossings and GC allocations.

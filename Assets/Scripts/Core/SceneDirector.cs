...
namespace Milehigh.Core
{
    public class SceneDirector : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            {
                var currentCampaignData = CampaignManager.Instance.currentCampaignData;
                if (currentCampaignData.scenarios != null && currentCampaignData.scenarios.Count > 0)
                {
                    SetupScene(currentCampaignData.scenarios[0]);
                }
            }
        }
        ...
        public void SetupScene(SceneScenario scenario)
        {
            ...
            {
                var currentCampaignData = CampaignManager.Instance.currentCampaignData;
                if (currentCampaignData.characters != null)
                {
                    foreach (var charProfile in currentCampaignData.characters)
                    {
        ...
        private void SpawnOrUpdateCharacter(CharacterProfile profile)
        {
            ...
            if (characterObj == null)
            {
                ...
                // Final fallback using the helper
                if (prefab == null)
                {
                    prefab = GetPrefab(profile.name);
                }

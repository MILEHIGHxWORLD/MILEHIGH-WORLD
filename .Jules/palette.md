...
## 2026-07-15 - [Preserve Unsent Draft Input During Terminal History Navigation]
**Learning:** In command-line interfaces, navigating up into history while typing an unsent command can cause frustration if the draft text is lost. Preserving unsent typed text in a dedicated `_inputBuffer` when moving away from the active line—and restoring it when returning down—provides a much more forgiving and natural terminal interaction loop.
**Action:** Always buffer active unsent input when initiating history navigation in CLI interfaces and restore it when returning to the current prompt line.

...
## 2026-07-15 - [Escape Key Line Clearing in Terminal UIs]
**Learning:** Power users and keyboard-driven users in terminal interfaces expect standard CLI line-clearing shortcuts like Escape. Clearing the active input line and resetting suggestion state on Escape—only when the input field is non-empty—improves input efficiency without disrupting higher-level UI keybindings (like pause menus).
**Action:** Implement guarded Escape key line clearing in CLI components and update command documentation to clearly distinguish between clearing output (Ctrl+L) and clearing line (Esc).

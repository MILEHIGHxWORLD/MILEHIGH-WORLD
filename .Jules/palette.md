...
## 2026-07-22 - [Persistent Input Buffer for CLI History]
**Learning:** In terminal-style interfaces, losing unsubmitted text when navigating history with arrow keys is a common friction point. Implementing a persistent 'live' input buffer that saves the user's current draft when they move away from the bottom of the history, and restores it when they return, significantly improves user confidence and flow.
**Action:** Always implement an input buffer to preserve unsubmitted text during CLI history navigation.

## 2026-06-18 - [Consistent Layout and Zero-Allocation Typewriter Architecture]
**Learning:** Resolving extreme class clutter and duplicate definitions in cinematic dialogue controllers is necessary to ensure consistent UX behavior. Integrating smart look-ahead pacing (skipping pauses on periods inside character names like 'Sky.ix') while pre-appending progression cues (like '▽') directly to the TMPro string ensures layout stability, preventing jarring visual line jumps while maintaining a highly professional reading flow.
**Action:** Unify all fragmented input and typewriter loops into single robust routines, pre-append all layout-impacting symbols before rendering, and leverage look-ahead checks to distinguish semantic from structural punctuation.

...
## 2026-08-05 - Missing Authentication on Narrative Action Endpoint
**Vulnerability:** The narrative action resolution endpoint request in NarrativeActionResolver.cs was missing an Authorization header, allowing unauthenticated requests to the backend API.
**Learning:** External API endpoints that resolve critical game states require authentication to prevent unauthorized manipulation or abuse.
**Prevention:** Always source and include secure tokens (like MILEHIGH_API_TOKEN) in API requests via headers.

...
## 2026-07-02 - Missing IDOR Protection for LatticeSynchronizer
**Vulnerability:** The `LatticeSynchronizer` core singleton was missing from the `_protectedManagers` blocklist in `SceneDirector.cs`.
**Learning:** New core managers added to the project, such as `LatticeSynchronizer`, are often overlooked when manually maintaining the IDOR protection blocklists, leaving them exposed to manipulation.
**Prevention:** Always verify that newly created core managers and singletons are added to the `_protectedManagers` blocklist in `SceneDirector.cs` during implementation.

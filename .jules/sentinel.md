...
## 2026-07-21 - Missing Authentication on API Endpoint
**Vulnerability:** The narrative action resolution endpoint (https://api.milehigh.world/v1/udp/resolve-action) in NarrativeActionResolver.cs was missing an Authorization header.
**Learning:** API requests to sensitive external endpoints must always include authentication tokens. It was relying on an unauthenticated client-side request.
**Prevention:** Always source secrets like MILEHIGH_API_TOKEN from environment variables using System.Environment.GetEnvironmentVariable and ensure they are attached as Bearer tokens in headers.

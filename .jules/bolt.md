...
## 2026-07-31 - [Hoist Lookups and Property IDs Outside Frame-Bound Loops]
**Learning:** In Unity, executing GetComponent<T>(), dictionary lookups (e.g., GetAlly), and string-based property updates (like Material.SetFloat) inside a frame-bound loop (such as an async while with Task.Yield()) causes significant CPU and string hashing overhead per frame due to native/managed boundary crossings.
**Action:** Always hoist and cache component lookups, dictionary query results, and Shader property IDs (via Shader.PropertyToID) outside performance-critical loops to minimize redundant overhead.

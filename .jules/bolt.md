...
## 2026-08-04 - Hoist Frame-Bound Lookups Outside Async Loops
**Learning:** In Unity, calling `GetComponent<T>()`, executing dictionary lookups, or performing string-based property updates (e.g., `Material.SetFloat("...")`) inside an async `while` loop containing `await Task.Yield()` causes significant per-frame CPU overhead and string hashing overhead due to native/managed boundary crossings.
**Action:** Always hoist and cache these lookups outside the loop. When calling `Shader.PropertyToID(string)`, always cache the returned `int` outside performance-critical loops to eliminate redundant string hashing per frame.

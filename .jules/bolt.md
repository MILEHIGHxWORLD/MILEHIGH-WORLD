...
## 2026-07-20 - Frame-Bound Component and Shader Overheads
**Learning:** In Unity, executing `GetComponent<T>()`, dictionary lookups (e.g., `GetAlly`), or string-based property updates (`Material.SetFloat("...")`) inside a frame-bound loop (like an async `while` loop containing `await Task.Yield()`) causes significant per-frame CPU overhead and string hashing overhead due to native/managed boundary crossings.
**Action:** Always hoist and cache these lookups, and use `Shader.PropertyToID(string)`, outside the performance-critical loops to minimize redundant execution and overhead.

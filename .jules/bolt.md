...
## 2026-07-20 - [Hoist Lookups and Property IDs outside Frame-Bound Loops]
**Learning:** In Unity, calling `GetComponent<T>()`, executing dictionary lookups (e.g., `GetAlly`), or performing string-based property updates (e.g., `Material.SetFloat("...")`) inside a frame-bound loop, such as an async `while` loop containing `await Task.Yield()`, causes significant per-frame CPU overhead and string hashing overhead due to native/managed boundary crossings.
**Action:** Always hoist and cache these lookups (components, dictionary values, and `Shader.PropertyToID`) outside the loop to eliminate redundant traversals and string hashing overhead per frame.

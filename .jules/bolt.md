...
## 2026-07-22 - Prevent GC and CPU Overhead in Async Loops
**Learning:** Executing GetComponent, dictionary lookups (e.g., GetAlly), or Shader.PropertyToID inside a frame-bound loop (like an async while loop with Task.Yield()) causes significant per-frame CPU overhead and string hashing overhead due to native/managed boundary crossings.
**Action:** Always hoist and cache these lookups outside the loop to eliminate redundant overhead.

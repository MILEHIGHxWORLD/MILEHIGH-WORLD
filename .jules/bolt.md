...
## 2026-07-25 - Hoist operations outside of frame-bound async loops
**Learning:** Calling `GetComponent<T>()`, executing dictionary lookups (like `director.GetAlly`), and performing string-based property updates (e.g., `Material.SetFloat`) inside a `while` loop with `await Task.Yield()` in `EndGameMultiFrontOrchestrator.cs` causes significant per-frame CPU overhead and string hashing overhead due to native/managed boundary crossings.
**Action:** Always hoist and cache these lookups (using `Shader.PropertyToID` for materials) outside the loop.

...
## 2026-08-02 - Cache Lookups Outside Frame-Bound Loops
**Learning:** In Unity, calling GetComponent<T>(), executing dictionary lookups (like GetAlly), or performing string-based property updates (e.g., Material.SetFloat("...")) inside a frame-bound loop, such as an async while loop containing await Task.Yield(), causes significant per-frame CPU overhead and string hashing overhead due to native/managed boundary crossings.
**Action:** Always hoist and cache these lookups outside the loop. When calling Shader.PropertyToID(string) in Unity, always hoist and cache the returned int outside of performance-critical loops to eliminate redundant string hashing overhead per frame.

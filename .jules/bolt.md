...
## 2024-05-27 - Unity Negative Caching Pitfalls in Scene Setup
**Learning:** Storing `null` (negative caching) or relying on complex `ReferenceEquals` checks in a GameObject dictionary cache adds unnecessary overhead and risks returning stale nulls for newly instantiated objects during iterative scene setups.
**Action:** Always use the simple `if (_cache.TryGetValue(key, out obj) && obj != null)` pattern for O(1) lookups. Only cache valid `GameObject.Find` results to robustly handle natively destroyed objects without negative caching complexities.

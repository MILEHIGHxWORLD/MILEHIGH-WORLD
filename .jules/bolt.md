...
## 2026-07-06 - [Hoisting Expensive Lookups Out of Async Loops]
**Learning:** In Unity, executing `GetComponent<T>()`, dictionary lookups (e.g., `GetAlly`), or string-based property updates (e.g., `Material.SetFloat("...")`) inside frame-bound loops (like an async `while` loop with `await Task.Yield()`) causes significant per-frame CPU and string hashing overhead due to native/managed boundary crossings.
**Action:** Always hoist and cache these lookups (`GetComponent`, dictionary values, and `Shader.PropertyToID`) outside of performance-critical loops to minimize redundant execution and overhead.

...
## 2026-07-19 - [Hoist Per-Frame Lookups in Async Loops]
**Learning:** Calling `GetComponent<T>()`, dictionary lookups (e.g., `GetAlly`), or string-based material property updates (e.g., `Material.SetFloat("...")`) inside a frame-bound async `while` loop containing `await Task.Yield()` causes significant per-frame CPU and string hashing overhead due to native/managed boundary crossings.
**Action:** Always hoist and cache these lookups (including `Shader.PropertyToID`) outside of frame-bound loops to eliminate redundant overhead and preserve performance.

...
## 2026-07-21 - [Hoist Lookups from Async Loops]
**Learning:** In Unity, executing `GetComponent<T>()`, dictionary lookups (like `GetAlly`), or string-based property updates (like `Material.SetFloat("...")`) inside an async `while` loop containing `await Task.Yield()` causes significant per-frame CPU overhead and string hashing overhead due to native/managed boundary crossings.

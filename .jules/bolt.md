...
## 2026-07-11 - Hoisting overhead outside frame-bound async loops
**Learning:** Calling `GetComponent<T>()`, executing dictionary lookups, or using string-based property updates (like `Material.SetFloat("string")`) inside an asynchronous `while` loop containing `await Task.Yield()` introduces massive per-frame CPU overhead due to native/managed boundary crossings and string hashing.
**Action:** Always hoist and cache these lookups outside of frame-bound loops to eliminate redundant overhead and preserve performance.

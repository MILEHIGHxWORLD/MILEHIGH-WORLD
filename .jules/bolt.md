...
## 2026-07-30 - Cache lookups outside async frame loops
**Learning:** In Unity, `GetComponent`, dictionary lookups (`GetAlly`), and string-based property updates (`SetFloat`) inside `while` loops containing `await Task.Yield()` cause significant per-frame CPU overhead and string hashing overhead due to native/managed boundary crossings.
**Action:** Always hoist and cache these lookups and property IDs outside the loop.

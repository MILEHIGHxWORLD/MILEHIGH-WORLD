...
## 2026-07-12 - Eliminate GC in WFC chunk generation
**Learning:** In asynchronous chunk generation (like Wave Function Collapse), repeatedly allocating local collections (`List<T>`) inside inner loops generates massive GC overhead and degrades performance.
**Action:** Replace dynamic list allocations with single-variable trackers or static readonly arrays in deterministic selection algorithms to avoid GC allocations entirely.

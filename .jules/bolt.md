...
## 2024-05-25 - Unified Caching in SceneDirector
**Learning:** Fragmented caching logic and redundant linear searches in 'SceneDirector.cs' caused significant initialization latency. Replacing (C \times P)$ linear prefab searches with (1)$ dictionary lookups yielded a ~4.5x speedup in character spawning logic. Robust negative caching (storing 'null') for missing scene objects further reduced overhead by preventing repeated (N)$ scene traversals during scenario setup.
**Action:** Always unify caching systems into a single O(1) flow. Use negative caching for known-missing assets and ensure caches are explicitly cleared in 'OnDestroy' to prevent stale reference leaks between scene loads.

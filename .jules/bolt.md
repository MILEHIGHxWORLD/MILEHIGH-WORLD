...
## 2024-07-04 - [Hoist Property Lookups in EndGameMultiFrontOrchestrator]
**Learning:** In Unity, executing dictionary lookups (e.g., `GetAlly`), performing string-based property updates (e.g., `Material.SetFloat("...")`), or calling `GetComponent<T>()` inside an async `while` loop containing `await Task.Yield()` causes significant per-frame CPU overhead and string hashing overhead due to native/managed boundary crossings.
**Action:** Always hoist and cache these lookups outside performance-critical loops to eliminate redundant string hashing and dictionary traversal overhead per frame.

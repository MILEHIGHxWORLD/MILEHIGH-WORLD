...
## 2026-07-28 - [Hoist Lookups Outside Async Frame-Bound Loops]
**Learning:** In Unity, calling GetComponent<T>(), executing dictionary lookups, or performing string-based property updates inside an async while loop containing await Task.Yield() causes significant per-frame CPU overhead due to native/managed boundary crossings.
**Action:** Always hoist and cache these lookups outside the loop to eliminate redundant execution paths and string hashing per frame.

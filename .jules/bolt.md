...
## 2026-07-07 - Hoisting Expensive Lookups in Async Loops
**Learning:** Executing dictionary lookups (like `director.GetAlly`), `GetComponent` calls, or string-based shader property updates (like `Material.SetFloat("string")`) inside a frame-bound async loop (e.g., `while` with `await Task.Yield()`) causes significant C#/C++ boundary crossing overhead and string hashing overhead per frame.
**Action:** Always hoist `GetComponent` and dictionary lookups outside of tight loops, and cache shader property IDs using `Shader.PropertyToID` at the class level to eliminate redundant frame overhead.

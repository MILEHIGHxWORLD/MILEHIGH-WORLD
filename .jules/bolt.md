...
## 2026-08-05 - Hoist Operations Outside Frame-Bound Loops
**Learning:** In Unity, executing `GetComponent<T>()`, dictionary lookups (e.g., `GetAlly`), and string-based property conversions (`Shader.PropertyToID`) inside an asynchronous `while` loop containing `await Task.Yield()` causes significant per-frame CPU overhead and string hashing overhead due to redundant native/managed boundary crossings.
**Action:** Always hoist and cache these lookups into local variables outside of performance-critical loops (like `Update` or `while` with `Task.Yield()`) to eliminate redundant per-frame overhead.

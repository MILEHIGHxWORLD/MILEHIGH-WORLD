...
## 2026-08-07 - [Hoist Lookups Outside Async Frame Loops]
**Learning:** Calling `GetComponent<T>()`, executing dictionary lookups, or performing string-based property updates (e.g., `Material.SetFloat("...")`) inside a frame-bound async `while` loop containing `await Task.Yield()` causes significant per-frame CPU overhead and string hashing overhead due to native/managed boundary crossings.

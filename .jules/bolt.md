...
## 2026-07-12 - Hoisting Lookups Outside Async Yield Loops
**Learning:** Native/managed boundary crossings in Unity (such as `GetComponent`, string-based `Material.SetFloat` lookups, and dictionary lookups) inside async `while` loops containing `await Task.Yield()` create severe per-frame CPU overhead and string hashing pressure due to repeated execution.
**Action:** Always hoist and cache `GetComponent` results, dictionary lookups, and `Shader.PropertyToID` integer keys outside of performance-critical frame-bound loops to eliminate redundant allocations and overhead.

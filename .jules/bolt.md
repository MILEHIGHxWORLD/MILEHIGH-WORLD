...
## 2026-08-03 - [Cache Component Lookups Outside Loops]
**Learning:** In Unity, calling GetComponent<T>(), executing dictionary lookups, or performing string-based property updates (e.g., Material.SetFloat("...")) inside a frame-bound loop, such as Update() or an async while loop containing await Task.Yield(), causes significant per-frame CPU overhead and string hashing overhead due to native/managed boundary crossings.
**Action:** Always hoist and cache these lookups outside the loop. Additionally, when calling Shader.PropertyToID(string), always hoist and cache the returned int outside of performance-critical loops to eliminate redundant string hashing overhead per frame.

...
## 2026-07-08 - [Material.SetFloat String Hashing Overhead in Frame-bound Loops]
**Learning:** In Unity, executing string-based property updates (e.g., Material.SetFloat("...")) inside a frame-bound loop, such as an async while loop containing await Task.Yield(), causes significant per-frame CPU overhead and string hashing overhead due to native/managed boundary crossings.
**Action:** Always hoist and cache the returned int from Shader.PropertyToID(string) outside of performance-critical loops, and use the integer overload for property updates to eliminate redundant string hashing overhead per frame.

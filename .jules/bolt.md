...
## 2026-07-13 - Hoist expensive lookups from Task.Yield loops
**Learning:** Calling GetComponent, dictionary lookups (e.g. GetAlly), or performing string-based property updates (e.g., Material.SetFloat) inside an async while loop containing Task.Yield() causes significant per-frame CPU and string hashing overhead due to native/managed boundary crossings.

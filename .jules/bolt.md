...
## 2026-06-21 - Coroutine GC Spikes in UI Typewriter Effects
...
## 2026-06-21 - [Hoisting Expensive Lookups out of Loops]
**Learning:** Calling `GetComponent<T>()` or `Material.SetFloat("...")` inside an async `while` loop containing `await Task.Yield()` causes significant per-frame CPU overhead and string hashing overhead due to native/managed boundary crossings.
**Action:** Always hoist and cache these lookups and `Shader.PropertyToID` conversions outside the loop to eliminate the overhead.

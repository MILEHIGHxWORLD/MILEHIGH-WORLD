...
## 2026-07-09 - Hoisting Lookups Outside Frame-Bound Loops
**Learning:** Calling GetComponent, dictionary lookups, and string-based property updates (Material.SetFloat) inside a frame-bound loop (Update or while with Task.Yield) causes significant per-frame CPU and string hashing overhead due to native/managed boundary crossings.
**Action:** Always hoist and cache these lookups outside the loop, caching integer property IDs with Shader.PropertyToID and storing component/entity references prior to loop execution.

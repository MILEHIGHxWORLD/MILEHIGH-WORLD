...
## 2026-07-19 - Frame-Bound Native Boundary Crossing Overhead
**Learning:** In Unity, executing `GetComponent<T>()`, dictionary lookups (like `GetAlly`), or performing string-based property updates (e.g., `Material.SetFloat("...")`) inside an async `while` loop containing `await Task.Yield()` causes significant per-frame CPU overhead and string hashing overhead due to native/managed boundary crossings.
**Action:** Always hoist and cache these lookups outside the loop using `Shader.PropertyToID` for properties and single variable assignments for components/references.

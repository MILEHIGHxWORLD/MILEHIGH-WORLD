...
## 2024-10-27 - MaterialPropertyBlock for Zero-Allocation Rendering
**Learning:** Accessing `Renderer.material` at runtime instantiates a clone of the material on the heap, generating GC allocations and breaking draw call batching (SRP/GPU instancing).
**Action:** Always use a cached `MaterialPropertyBlock` (via `GetPropertyBlock` and `SetPropertyBlock`) with cached property IDs (`Shader.PropertyToID`) for per-renderer shader modifications to ensure zero-allocation overhead.

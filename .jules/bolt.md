...
## 2026-07-11 - Hoist Lookups and String Hashes in Async Unity Loops
**Learning:** In Unity, executing `GetComponent<T>()`, dictionary lookups, or string-based property updates (`Material.SetFloat`) inside frame-bound loops like an async `while` loop containing `await Task.Yield()` causes significant per-frame CPU overhead and string hashing overhead due to native/managed boundary crossings.
**Action:** Always hoist and cache these lookups outside the loop. Use `Shader.PropertyToID(string)` to cache material property IDs as integers before the loop instead of passing strings.

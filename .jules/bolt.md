...
## 2026-07-28 - [Frame-Bound Loop Lookup Hoisting]
**Learning:** Calling GetComponent<T>(), executing dictionary lookups (e.g., GetAlly), or performing string-based property updates (e.g., Material.SetFloat) inside a frame-bound loop, such as Update() or an async while loop containing await Task.Yield(), causes significant per-frame CPU overhead and string hashing overhead due to native/managed boundary crossings.
**Action:** Always hoist and cache these lookups, using Shader.PropertyToID for shader properties, outside of frame-bound loops to eliminate redundant overhead per frame.

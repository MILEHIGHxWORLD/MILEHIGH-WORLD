...
## 2024-05-26 - Float Keys in Dictionary Caches
**Learning:** Using `float` values as keys in C# Dictionaries (like `Dictionary<float, WaitForSeconds>`) leads to cache misses due to floating-point precision variations. This defeats the purpose of the cache and causes unexpected Garbage Collection (GC) allocations.
**Action:** Always convert float values to an integer representation (like milliseconds via `Mathf.RoundToInt(time * 1000f)`) before using them as dictionary keys.

...
## 2026-07-22 - Cache Lookups Outside Frame-Bound Loops
**Learning:** Calling `GetComponent<T>()`, executing dictionary lookups, or performing string-based property updates (e.g., `Material.SetFloat("...")`) inside an async `while` loop containing `await Task.Yield()` causes significant per-frame CPU overhead and string hashing overhead due to native/managed boundary crossings.
**Action:** Always hoist and cache these lookups and integer property IDs (using `Shader.PropertyToID`) outside the loop to eliminate redundant per-frame overhead and string hashing.

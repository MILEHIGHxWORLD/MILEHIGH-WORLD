...
## 2024-05-26 - [WaitForSeconds Cache Misses]
**Learning:** When caching `WaitForSeconds` in a `Dictionary` using a `float` as the key, floating-point tolerance variations can cause cache misses, leading to unnecessary GC allocations.
**Action:** Use an `int` key representing milliseconds (e.g., via `Mathf.RoundToInt(time * 1000f)`) rather than `float` to ensure reliable dictionary lookups and prevent cache misses.

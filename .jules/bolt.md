...
## 2026-07-29 - Caching Component Lookups and Shader Property IDs Outside Frame-bound Loops
**Learning:** In Unity, calling `GetComponent<T>()`, executing dictionary lookups (e.g. `GetAlly`), or performing string-based property updates (e.g. `Material.SetFloat("...")`) inside a frame-bound loop, such as an async `while` loop containing `await Task.Yield()`, causes significant per-frame CPU overhead and string hashing overhead due to C#/C++ native boundary crossings.
**Action:** Always hoist and cache these lookups outside the loop.

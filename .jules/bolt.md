...
## 2026-08-05 - Coroutine GC Spikes in UI Typewriter Effects
...
## 2026-08-05 - Hoisting Expensive Operations from Frame-bound Loops
**Learning:** Calling GetComponent<T>(), doing dictionary lookups (like GetAlly), and using string-based shader property updates (e.g., Material.SetFloat) inside frame-bound loops (like an async while loop with Task.Yield()) causes immense overhead per frame due to native/managed boundary crossings and string hashing.
**Action:** Always hoist and cache component references, dictionary lookup results, and Shader.PropertyToID integers outside of per-frame loops to prevent CPU spiking and GC allocations.
